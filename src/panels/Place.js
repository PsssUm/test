  ...
  return (
    <div className="Place">
      ...
      <footer className="Place__footer">
        <Link style={price != 0 ? {} : {pointerEvents : 'none', opacity : '0.5'}} onClick={() => {if (price != 0){window.location.href = `/basket/${area.id}/${item.id}`}}} to='#' className="Place__order">
          {price != 0 ? "Оформить заказ (" + price + ")" : "Корзина пустая"}
        </Link>
      </footer>
    </div>
  );
